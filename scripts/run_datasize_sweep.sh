...
# 输出文件: output/datasize_sweep_results.txt
...
EOF
  ...
  start_time=$(date +%s)
  result=$(/share_data/zhuyu/chiplet-network-sim/build/ChipletNetworkSim /share_data/zhuyu/chiplet-network-sim/input/test_sweep.ini 2>&1 | grep -E "Data size|Throughput" | tail -2)
  end_time=$(date +%s)
  elapsed=$((end_time - start_time))
  
  throughput=$(echo "$result" | grep "Throughput" | grep -oP "[0-9.]+")
  cycles=$(echo "$result" | grep "Data size" | grep -oP "Cycles: \K[0-9]+")
