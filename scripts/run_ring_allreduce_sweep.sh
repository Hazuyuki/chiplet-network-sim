#!/bin/bash
# ring_all_reduce data_size 扫描实验

OUTPUT_FILE="/share_data/zhuyu/chiplet-network-sim/output/ring_allreduce_sweep_results.txt"
LOG_FILE="/share_data/zhuyu/chiplet-network-sim/output/ring_allreduce_sweep.log"

echo "=== ring_all_reduce data_size 扫描测试 (min 路由) ===" | tee $OUTPUT_FILE
...
for data_size in 1024 2048 4096 8192 16384 32768 65536 131072 262144; do
  cat > /share_data/zhuyu/chiplet-network-sim/input/test_ring_ar.ini << EOF
[Network]
topology = NVSwitch
num_spines = 4
num_leafs = 4
...
[Files]
output_file = output/test_ring_ar.csv
EOF
  
  echo "Running data_size = $data_size..." | tee -a $LOG_FILE
  start_time=$(date +%s)
  result=$(/share_data/zhuyu/chiplet-network-sim/build/ChipletNetworkSim /share_data/zhuyu/chiplet-network-sim/input/test_ring_ar.ini 2>&1)
  end_time=$(date +%s)
