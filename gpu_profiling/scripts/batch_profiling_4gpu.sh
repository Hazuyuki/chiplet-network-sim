...
# 只使用GPU 0-3（避免GPU 4-7被占用）
export CUDA_VISIBLE_DEVICES=0,1,2,3

# 测试参数 - 从小数据到大数据，直到带宽收敛
DATA_SIZES=("128KB" "256KB" "512KB" "1MB" "2MB" "4MB" "8MB" "16MB" "32MB" "64MB" "128MB" "256MB" "512MB" "1024MB" "2048MB" "4096MB" "8192MB" "16384MB" "32768MB" "65536MB")
ITERATIONS=100
WARMUP=10
NUM_GPUS=4  # 使用 4 GPU
...
MAIN_OUTPUT_DIR="${RESULTS_DIR}/batch_4gpu_${TIMESTAMP}"
mkdir -p "${MAIN_OUTPUT_DIR}"

echo -e "${GREEN}=== Batch NCCL All-Reduce Profiling (4 GPUs) ===${NC}"
...
for SIZE in "${DATA_SIZES[@]}"; do
    ...
    if [ -f "$LOG_FILE" ]; then
        ...
        if [ -n "$AVG_TIME" ] && [ -n "$BANDWIDTH" ]; then
            # A800 单 GPU 单向理论带宽 200 GB/s
            EFFICIENCY=$(python3 -c "print(round($BANDWIDTH / 200 * 100, 1))")
            printf "| %-10s | %-12s | %-15s | %-15s | %-10s%%|\n" "$SIZE" "$AVG_TIME" "$BANDWIDTH" "200 GB/s" "$EFFICIENCY"
        else
            printf "| %-10s | %-12s | %-15s | %-15s | %-10s|\n" "$SIZE" "N/A" "N/A" "200 GB/s" "N/A"
        fi
    else
        printf "| %-10s | %-12s | %-15s | %-15s | %-10s|\n" "$SIZE" "No log" "No log" "200 GB/s" "No log"
    fi
done
